...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function handleNetomiMessage(message) {
        try {
            ...
            // Get conversation ID (already generated by GTM Manager on page load)
            if (!window.netomiConversationId) {
                // Fallback: get from sessionStorage or generate if needed
                window.netomiConversationId = sessionStorage.getItem('netomiConversationId') || `chat-${Date.now()}`;
            }
